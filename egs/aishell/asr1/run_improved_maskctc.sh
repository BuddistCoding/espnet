...
train_config=conf/tuning/train_transformer_improved_maskctc.yaml
lm_config=conf/lm.yaml
decode_config=conf/tuning/decode_transformer_improved_maskctc.yaml
...
if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
    ...
    if [[ $(get_yaml.py ${train_config} model-module) = *transducer* ]]; then
        ...
        recog_v2_opts=""
    elif [ ${skip_lm_training} ]; then
        echo "skip language model name"
        recog_v2_opts=""
    else
