...
class CrossAttEncoderLayer(nn.Module):
    """Sep_Encoder layer module.
    Only used when q and k are different source 
    ...
    def __init__(
        ...
        """Construct an EncoderLayer object."""
        super(CrossAttEncoderLayer, self).__init__()
        self.self_attn = self_attn
        self.feed_forward = feed_forward
        self.norm1 = LayerNorm(size)
        self.norm2 = LayerNorm(size)
    ...
    def forward(self, x, x_mask, memory = None, memory_mask = None, cache=None):
        ...
        Args:
            x_input (torch.Tensor): Input tensor (#batch, time, size).
            h (torch.Tensor): another input for key in attention (#batch, time, hidden size)
            mask (torch.Tensor): Mask tensor for the input (#batch, time).
            cache (torch.Tensor): Cache tensor of the input (#batch, time - 1, size).
        ...
        if memory is None and memory_mask is None:
            ...
            if cache is None:
                ...
                x_q = x[:, -1:, :]
                residual = residual[:, -1:, :]
                x_mask = None if x_mask is None else x_mask[:, -1:, :]

            if self.concat_after:
                x_concat = torch.cat((x, self.self_attn(x_q, x, x, x_mask)), dim=-1)
                x = residual + stoch_layer_coeff * self.concat_linear(x_concat)
            else:
                x = residual + stoch_layer_coeff * self.dropout(
                    self.self_attn(x_q, x, x, x_mask)
                )
            if not self.normalize_before:
                x = self.norm1(x)
