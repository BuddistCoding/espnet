...
from espnet.nets.pytorch_backend.maskctc.add_mask_token import mask_uniform, mask_uniform_dlp, shrink, insert_mask, expand
...
class DLPReporter(chainer.Chain):
    def report(self, loss_ctc, loss_att, acc, loss_dlp, acc_dlp , cer_ctc, cer, wer, mtl_loss):
        ...
        reporter.report({"mtl_loss": mtl_loss}, self)
...
class E2E(E2ETransformer):
    ...
    def __init__(self, idim, odim, args, ignore_id=-1):
        ...
        assert 0.0 <= self.mtlbeta, "mtlbeta should be non-negative"
        ...
        self.dlp = nn.Linear(args.adim, 50) # What number?
        self.dlp_criterion = LabelSmoothingLoss(50, self.ignore_id, 0.0, False)
    ...
    def forward(self, xs_pad, ilens, ys_pad):
        ...
        pred_pad = self.decoder_output_layer(pred_pad) ## important(why ??)
        ...
        # 4. compute insertion / deletion error
        ...
        pred_pad_dlp = self.dlp(pred_pad_dlp)
       
        self.loss_dlp = self.dlp_criterion(pred_pad_dlp, ys_out_pad_dlp)
        ...
        self.loss =  (self.loss + beta * self.loss_dlp)

        loss_dlp_data = float(self.loss_dlp)
...
# =========== origin iterative decoding ==========
        ...
        K = recog_args.maskctc_n_iterations
        if K > 0:
            num_decode = max( (mask_num // K) , 1)
        else:
            num_decode = 1

        # shrink-and-expand algorithm
        
        logging.info(f"predict {num_decode} in each iteration.")
        ...
        while len(mask_idx) > 0:
            ...
            mask_idx = torch.nonzero(y_in[0] == self.mask_token).squeeze(-1)
            logging.info(f'shr:{n2s(y_in[0].tolist())}')
            ...
            y_in = y_tmp.long()
            mask_idx = torch.nonzero(y_in[0] == self.mask_token).squeeze(-1)
            logging.info(f"exp{n2s(y_in[0].tolist())}")
            ...
            _, cand = torch.topk(pred_y_score, min(num_decode, len(mask_idx)), -1)
            y_in[0][mask_idx[cand]] = pred_y_id[cand]
            mask_idx = torch.nonzero(y_in[0] == self.mask_token).squeeze(-1)
            logging.info(f'pre:{n2s(y_in[0].tolist())}')
        
        logging.info("------------")
        logging.info(f"ctc:{n2s(y_hat[y_idx].tolist())}")
        logging.info(f"msk:{n2s(y_in[0].tolist())}")
        logging.info("------------")

            # y_in, _ = shrink(y_in, self.mask_token, self.eos, self.ignore_id)
            # shrink_idx = torch.nonzero(y_in == self.mask_token)
            # y_dlp = torch.zeros(y_in.shape[1]).fill_(self.ignore_id).to(y_in)
            # pred, _ = self.decoder(y_in, None, h, None)
            # _ , dlp_length = self.dlp(pred)[0].max(dim = -1)
            # y_dlp[shrink_idx] = dlp_length[shrink_idx]

            # y_in = expand(y_in, y_dlp.unsqueeze(0), self.mask_token, self.eos, self.ignore_id)
            # mask_idx = torch.nonzero(y_in[0] == self.mask_token).squeeze(-1)
            
            # logging.info(f'after_expand:{y_in}')
            # if (len(mask_idx) == 0):
            #     break
            # mask_num = len(mask_idx)
            # pred, _ = self.decoder(y_in, None, h, None)
            # pred_score, pred_idx = pred[0][mask_idx].max(dim = -1)
            # cand = torch.topk(pred_score, min(num_decode, len(mask_idx)), -1)[1]
            
            # y_in[0][mask_idx[cand]] = pred_idx[cand]

            
            # mask_idx == torch.nonzero(y_in[0] == self.mask_token)
